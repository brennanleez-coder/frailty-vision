    ...
    return (
        <>
            <h2 className="text-2xl font-bold text-gray-600 mb-6 text-center">
                Gait Speed Walk Test Result {(average_speed.toFixed(2) > 100 && distance_walked.toFixed(2) >= 400) ? "✅" : "❌"} - {new Date().toLocaleDateString()}
            </h2>
            {/* Distance Walked and Elapsed Time */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className={`p-4 rounded-lg ${distance_walked === 5 ? 'bg-green-100' : 'bg-gray-100'}`}>
                    <h3 className="font-medium text-primary">Distance Walked (cm)</h3>
                    <p className="text-primary text-xl">{distance_walked.toFixed(2)}</p>
                </div>
                <div className={`p-4 rounded-lg ${average_speed.toFixed(2) > 100 ? 'bg-green-100' : 'bg-red-100'}`}>
                    <h3 className="font-medium text-primary">Average Speed (cm/s) </h3>
                    <p className="text-primary text-xl">{average_speed.toFixed(2)}</p>
                </div>
            </div>
